...
def data_loader( data_path, task_type): 
...
def tensor_dataset_transformers(prop, X_train, y_train, dtype = torch.float32):
    # input: 
    #   x [np.float32] - shape: (batch_size, seq_len, feature_size)  
    #   y [np.float32] - shape: (batch_size)  value is according to nclasses;1-n


    num_inst = X_train.shape[0]
    num_train_samples = math.ceil(num_inst / prop['batch']) * prop['batch']
    # num_test_samples = math.ceil(num_test_inst / prop['batch']) * prop['batch']
    
    # concatenate zeros in end of matrix (last batch) to make  batches with fixed size 
    X_train = make_perfect_batch(X_train, num_inst, num_train_samples)
...
def initialize_training(prop,dtype = torch.float32):
    model = multitask_transformer_class.MultitaskTransformerModel(prop['task_type'], prop['device'], prop['nclasses'], prop['seq_len'], prop['batch'], \
        prop['input_size'], prop['emb_size'], prop['nhead'], prop['nhid'], prop['nhid_tar'], prop['nhid_task'], prop['nlayers'], prop['dropout'],prop['get_attn_maps']).to(prop['device'])
    best_model = multitask_transformer_class.MultitaskTransformerModel(prop['task_type'], prop['device'], prop['nclasses'], prop['seq_len'], prop['batch'], \
        prop['input_size'], prop['emb_size'], prop['nhead'], prop['nhid'], prop['nhid_tar'], prop['nhid_task'], prop['nlayers'], prop['dropout'],prop['get_attn_maps']).to(prop['device'])
    model.type(dtype); best_model.type(dtype)#allow change model type(gradient precision) into half precision (float16) for faster training
...
def multitask_train(model, criterion_tar, criterion_task, optimizer, X_train_tar, X_train_task, y_train_tar_masked, y_train_tar_unmasked, \
    ...
    #transform list of all batches attention maps into single tensor  with dim (batch_len*num_batches, seq_len)   
...
def test(model, X, y, batch, nclasses, criterion, task_type, device, avg):
    ...
    with torch.no_grad():
        for i in range(num_batches):
            ...
            # notice the concatenated zeroes are not passed to the model due to num_inst shape
    ...
    return torch.cat(output_arr, 0), y, nclasses, criterion, task_type, device, avg
...
def training(model, optimizer, criterion_tar, criterion_task, best_model, best_optimizer, X_train_task, y_train_task, X_test, y_test, prop):
    ...
    acc_arr =[]#save accuracy of all epochs
    ...
    for epoch in range(1, prop['epochs'] + 1):
        ...
        # append the attention maps of all the epoch instances in the training set (add dimension for epoch instance)
        weights.append(instance_weights[None, :,:])
        ...
        y_pred, y, nclasses, criterion, task_type, device, avg = \
            test(model, X_test, y_test, prop['batch'], prop['nclasses'], criterion_task, prop['task_type'], prop['device'], prop['avg'])
        # test(best_model, X_test, y_test, prop['batch'], prop['nclasses'], criterion_task, prop['task_type'], prop['device'], prop['avg'])


        test_metrics = evaluate(y_pred, y, nclasses, criterion, task_type, device, avg)
        # returns test_metrics: for classification [loss, acc, prec, rec, f1] else [rmse, mae]
        acc_arr.append(test_metrics[1])
        ...
        if prop['task_type'] == 'classification' and test_metrics[1] > acc:
            ...
            rmse = test_metrics[0]
            mae = test_metrics[1]
        print('Epoch: ' + str(epoch) + ', TAR Loss: ' + str(tar_loss), ', TASK Loss: ' + str(task_loss)+ ',epoch Acc: ' + str(acc_arr[-1]) )
        if acc>=1.0:
            break

    if prop['task_type'] == 'classification':
        print('Dataset: ' + prop['dataset'] + ', best Acc: ' + str(acc))
    elif prop['task_type'] == 'regression':
        print('Dataset: ' + prop['dataset'] + ', RMSE: ' + str(rmse) + ', MAE: ' + str(mae))

    del model
    torch.cuda.empty_cache()
    return  torch.cat(weights, axis = 0),acc_arr
# --------------------------------------------------------------------------
