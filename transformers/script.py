...
# sys.path.insert(0,path + '/mutual_functions/' )
data_path = 'c:\\Users\\yishai\\Desktop\\classification_yishai/mutual_functions/data/'


parser = argparse.ArgumentParser()
parser.add_argument('--dataset', type=str, default='AF')
parser.add_argument('--batch', type=int, default=16)#128
...
def main(data_path):    
    ...
    prop['get_attn_maps'] = False
    path = data_path + prop['dataset'] + '/'
    
    print('Data loading start...')
    X_train, y_train, X_test, y_test = utils.data_loader( path, prop['task_type'])
    ...
    attention_maps,acc_arr = utils.training(model, optimizer, criterion_tar, criterion_task, best_model, best_optimizer, X_train_task, y_train_task, X_test, y_test, prop)
