...
class MultitaskTransformerModel(nn.Module):
    ...
    def __init__(self, task_type, device, nclasses, seq_len, batch, input_size, emb_size, nhead, nhid, nhid_tar, nhid_task, nlayers, dropout = 0.1,get_attn_maps = False):
        super(MultitaskTransformerModel, self).__init__()
        # from torch.nn import TransformerEncoder, TransformerEncoderLayer
        
        self.trunk_net = nn.Sequential(
            # transform only feature_len to emb_size
    ...
    def forward(self, x, task_type):
        # permute dim from (batch_len x seq_len x feature_len) to (seq_len x batch_len x feature_len)
        
        # transforming feature_len to emb_size (2->64) and than PositionalEncoding and batch normalizing (re-centering and re-scaling batch data)
        x = self.trunk_net(x.permute(1, 0, 2)) 
        x, attn = self.transformer_encoder(x)
        x = self.batch_norm(x)
        
        # current x.shape : seq_len x batch x emb_size
        if task_type == 'reconstruction':
            # return to original dim (batch_len x seq_len x feature_len)
            output = self.tar_net(x).permute(1, 0, 2)
        elif task_type == 'classification':
            # return (batch_sizeX1)  value is according to nclasses;1-n
