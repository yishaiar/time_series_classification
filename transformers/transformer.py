...
class TransformerEncoderLayer(nn.Module):
    ...
    def __init__(self, d_model, nhead, dim_feedforward = 2048, dropout = 0.1, activation = "relu", layer_norm_eps = 1e-5,get_attn_maps = False):
        ...
        self.get_attn_maps = get_attn_maps
    ...
    def forward(self, src, src_mask = None, src_key_padding_mask = None):
        ...
        """
        # forward: self_attn(query, key, value, key_padding_mask, attn_mask ) returns attn_output (prediction), attn_output_weights(attention maps)
        # key_padding_mask (Optional)  a mask indicating which elements within key to ignore for the purpose of attention ( treat as “padding”)
        # attn_mask says what key-value pairs are valid to attend to
        src2, attn = self.self_attn(src, src, src, attn_mask = src_mask,
                              key_padding_mask = src_key_padding_mask,
                              need_weights=self.get_attn_maps)
...
class TransformerEncoder(nn.Module):
    ...
    def forward(self, src, mask = None, src_key_padding_mask = None):
        ...
        for mod in self.layers:
            ...
            # every layer in TransformerEncoderLayer() class adds attention matrix to attn_output (total atention map of batch)
            if attn is not None:
                attn_output += attn
